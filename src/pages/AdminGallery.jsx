...
import { getCreations, createCreation, updateCreation, deleteCreation, incrementCreationView, toggleCreationLike } from "../api/creations";
...
const AdminGallery = () => {
  ...
  const loadGallery = useCallback(async () => {
    setIsLoading(true);
    try {
      const response = await getCreations({ scope: "gallery" });
  ...
  const filteredContent = useMemo(() => {
    ...
    const filteredItems = galleryContent.filter((item) => {
      const matchesFilter =
        activeFilter === "all"
          ? true
          : ["published", "draft", "shared", "private"].includes(activeFilter)
            ? item.status === activeFilter
            : item.hostingPlatform === activeFilter;
  ...
  }, [activeFilter, galleryContent, searchQuery, sortBy]);
  ...
  const handleSave = async (contentData) => {
    try {
      const resolvedScope =
        modalState.mode === "create" ? "gallery" : modalState.content?.scope || contentData.scope || "gallery";
      const payload = {
        ...contentData,
        scope: resolvedScope,
      };
      ...
      if (modalState.mode === "create") {
        ...
        showAlert("Gallery content created successfully", { type: "success", duration: 3000 });
      } else {
        const contentId = modalState.content?._id || contentData._id || contentData.id;
        await updateCreation(contentId, payload);
        showAlert("Gallery content updated successfully", { type: "success", duration: 3000 });
      }

      setModalState({ show: false, mode: "create", content: null });
      await loadGallery();
    } catch (error) {
      showAlert(error?.message || "Failed to save gallery content", {
        type: "error",
        duration: 5000,
      });
    }
  ...
  const handleDelete = async (id) => {
    try {
      ...
      setGalleryContent((prev) => prev.filter((item) => item._id !== id));
      showAlert("Gallery content deleted", { type: "success", duration: 3000 });
    } catch (error) {
      showAlert(error?.message || "Failed to delete gallery content", {
        type: "error",
        duration: 5000,
      });
    }
  ...
  const toggleVisibility = async (creation) => {
    const statusTransitions = creation.scope === "gallery"
      ? { published: "draft", draft: "published" }
      : { shared: "private", private: "shared" };
    const nextStatus = statusTransitions[creation.status] || (creation.scope === "gallery" ? "published" : "shared");
    ...
    try {
      ...
      if (creation.scope === "locker" && nextStatus === "private") {
        setGalleryContent((prev) => prev.filter((item) => item._id !== creation._id));
        setPreviewCreation((prev) => (prev && prev._id === creation._id ? null : prev));
      } else {
        setGalleryContent((prev) =>
          prev.map((item) => (item._id === creation._id ? { ...item, status: nextStatus } : item))
        );
        setPreviewCreation((prev) =>
          prev && prev._id === creation._id ? { ...prev, status: nextStatus } : prev
        );
      }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
      ...
      >
        <Header searchQuery={searchQuery} setSearchQuery={setSearchQuery} searchPlaceholder="Search gallery..." />
        ...
        <div className="p-4 md:p-6">
          ...
          {isLoading ? (
            <DataFetchLoader message="Loading gallery content..." />
                ...
                >
                  <div className="relative h-48 overflow-hidden">
                    ...
                    <div className="absolute top-4 right-4">
                      ...
                      >
                        {item.status === "shared" ? "public" : item.status}
                      </span>
                    </div>
                      ...
                      <div className="flex space-x-2">
                        ...
                        <button
                          ...
                          className={`p-2 transition-colors ${
                            ["shared", "published"].includes(item.status)
            ...
            >
              ...
              <h2 className="text-xl md:text-2xl font-bold text-white mb-2">Gallery is Empty</h2>
      ...
      {modalState.show && (
        <CreationModal
          ...
          creation={modalState.content}
          onClose={() => setModalState({ show: false, mode: "create", content: null })}
          onSave={handleSave}
          statusLabel={modalState.content?.scope === "locker" ? "Visibility" : "Publication"}
          statusOptions={
            modalState.content?.scope === "locker"
              ? [
